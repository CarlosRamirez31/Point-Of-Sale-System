namespace Utilities.Static
{
    public class ReplyMessage
    {
        public static string MESSAGE_QUERY = "Consulta exitosa.";
        public static string MESSAGE_QUERY_EMPTY = "No se encuentran registros.";
        public static string MESSAGE_SAVE = "Se registro correctamente.";
        public static string MESSAGE_UPDATE = "Se actualizo correctamente.";
        public static string MESSAGE_DELETE = "Se elimino correctamente.";
        public static string MESSAGE_EXISTS = "El registro ya existe.";
        public static string MESSAGE_ACTIVATE = "El registro se ha activado.";
        public static string MESSAGE_TOKEN = "Token generado correctamente.";
        public static string MESSAGE_VALIDATE = "Errores de validacion.";
        public static string MESSAGE_FAILED = "Validacion Fallida.";
    }
}
